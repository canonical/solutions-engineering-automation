...
PROJECTS: List[dict] = [
    ...
    {"name": "apt-mirror", "type": "charm"},
    {"name": "bcache-tuning", "type": "charm"},
    {"name": "hardware-observer", "type": "charm"},
    {"name": "juju-backup-all", "type": "charm"},
    {"name": "local-users", "type": "charm"},
    {"name": "logrotated", "type": "charm"},
    {"name": "memcached", "type": "charm"},
    {"name": "nginx", "type": "charm"},
    {"name": "nrpe", "type": "charm"},
    {"name": "openstack-service-checks", "type": "charm"},
    {"name": "prometheus-blackbox-exporter", "type": "charm"},
    {"name": "prometheus-libvirt-exporter", "type": "charm"},
    {"name": "rsyslog-forwarder-ha", "type": "charm"},
    {"name": "simple-streams", "type": "charm"},
    {"name": "sysconfig", "type": "charm"},
    {"name": "userdir-ldap", "type": "charm"},
    {"name": "juju-backup-all", "type": "snap"},
    {"name": "prometheus-juju-backup-all-exporter", "type": "snap"},
    {"name": "prometheus-libvirt-exporter", "type": "snap"},
...
SBOM_MATRIX: dict = {
    "clients": {
        "sbom": {
            ...
            "email": "solutions.engineering@canonical.com",
...
def handle_snap(name: str, risk: str, cycle: str, arch: str) -> dict:
    ...
    if not revision:
        raise ValueError(f"Revision not found for snap {name} on risk {risk}")
...
def handle_charm(name: str, risk: str, cycle: str, arch: str) -> dict:
    ...
    if not filtered_channels:
        raise ValueError(
            f"No channels found for charm {name} with architecture {arch} and track {charm_default_track}"
        )
    ...
    if not revision:
        raise ValueError(
            f"Revision not found for charm {name} on risk {risk} and architecture {arch}"
        )
...
def generate_matrix(risk: str, cycle: str, arch: str) -> dict:
    ...
    for project in PROJECTS:
        ...
        try:
            ...
            logger.error(f"Failed to process project {pname}: {e}")
            errors.append(f"{pname}: {e}")
            continue
        artifacts.append(artifact)

    if errors:
        logger.warning(f"The following projects failed to process: {errors}")
        exit(1)
